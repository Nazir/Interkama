...
Формирование данных о программном обеспечении (далее ПО) установленном на компьютерном оборудовании нижнекамского филиала ОАО «Ак Барс» Банк «Интеркама» в систему OTRS:ITSM (далее Система).

> Дата назначения: 19.01.2009  
> Дата начала: 19.01.2009  
> Дата окончания: -
> Исполнитель: Хуснутдинов Назир Каримович  

### Поиск пути решения ### 
В качестве источника данных была выбрана программа «WinAudit Freeware v2.13», которая уже давно введена в эксплуатацию.  
После анализа данных сформированных программой «WinAudit Freeware v2.13», было решено воспользоваться встроенным механизмом импорта/экспорта Системы. Для этого необходимо осуществить обработку и преобразование полученных данных в соответствии с форматами импорта/экспорта Системы.  

### Принятые меры: ### 

Этап 1: Обработка данных.  
Так как полученные данные имеют свою структуру отличную от структуры Системы, мною было решено  создать промежуточные таблицы и SQL-скрипты, котрые подготавливали бы полученые данные для импорта/экспорта в Систему.  

Программа «WinAudit Freeware v2.13» способна формировать набор данных в различных форматах:  
- CSV comma delimited values
- HTML web page without images
- HTMLi web page with images
- ODBC Export to a Database
- PDF portable document format
- TEXT formatted text
- TEXTt tab delimited text
- TEXTu unicode text (UTF-16, little endian)
- XML extended markup language
Был выбран CSV формат.

Этап 2: Корректировка данных в соответствии с форматом импорта/экспорта Системы.
К данным добавил обязательные для Системы поля «Состояние использования» и «Состояние инцидента», так как без них при импорте в Систему данные не импортируются и не выдаётся сообщение об ошибке (что привело к потере времени)!  
...
Так же был написан SQL-скрипт связки "Computer" с "Software".

### Промежуточные задачи ###

1. Обработка данных в формате DBF для программы «Реестр углублённых мед. осмотров». 
2. Скриншоты для банковских киосков (в папке «Screenshots\kiosk\»).
3. Настройка новых компьютеров (привлечение в связи большого объёма работ), перенос данных со старых компьютеров и установка стандартного программного обеспечения:
    1. Дежурные администраторы (к. 250) — 3 компьютера 
    2. Лушникова О.В. (к. 212)
    3. Сизова С.А. (к. 208)
4. Копирование документов по ипотечным кредитам (4 дела в день). Израсходовано 11 пачек бумаги (по 500 листов). Заняло примерно 3 недели.
